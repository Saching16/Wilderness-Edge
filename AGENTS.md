...
This build is being executed as an ~4-hour hackathon sprint. The scope trims below are deliberate engineering tradeoffs for the sprint, not a change to the project's long-term ambitions — see `plans/scope-design.md` for the full rationale and `plans/hackathon-sprint.md` (or the per-person plan files in the same directory) for the task breakdown.
...
9. **Sprint Task Ownership** — See `plans/hackathon-sprint.md` for the full task breakdown, or the per-person files in the same directory (`pablo.md`, `vaibhav.md`, `daniel.md`, `sachin.md`) for an individual's assigned tasks only.